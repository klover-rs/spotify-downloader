          ...
          if (newStatus) {
            setDownloadState((prev) => {
              const existingIndex = prev.findIndex(
                (item) => item.file_name === newStatus.file_name,
              );
              if (existingIndex !== -1) {
        ...
        <div key={index} className="download-item">
          ...
          {state.status === "Downloading" && (
            <>
              <p>
                Downloading:{" "}
                {Math.round(Number(state.downloaded_bytes) / 1000 / 1000)} /{" "}
                {Math.round(Number(state.total_bytes) / 1000 / 1000)} Mb
              </p>
              <div className="progress-bar">
                <div
                  className="progress"
                  style={{
                    width: `${(state.downloaded_bytes! / state.total_bytes!) * 100}%`,
                  }}
                />
              </div>
            </>
          )}
          {state.status === "Encoding" && <p>Encoding...</p>}
          {state.status === "Downloaded" && <p>Downloaded!</p>}
        </div>
      ))}
    </div>
  );
